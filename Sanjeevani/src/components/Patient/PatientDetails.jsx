...
import { useGetAppointmentsQuery } from "../../app/features/patient/PatientApiSlice";

function PatientDetails() {
  const [anchorEl, setAnchorEl] = React.useState(null);
  const data = useSelector((state) => state.patient);
  const { data: appointments } = useGetAppointmentsQuery(`${data.name}`);
      ...
      <div className="flex flex-row justify-around  p-5 bg-pink-400 ">
        ...
        <div className="w-6/12 bg-white rounded-3xl mt-5 ">
          ...
          <div className="lg:h-[90vh] h-[70vh] overflow-y-auto">
            {appointments?.map(
              ({ name, lastname, date, time, speacialist, src }) => (
                            ...
                            >
                              <div className="flex flex-col sm:w-60 sm:h-60 w-24 h-24 rounded-full">
                                ...
                                <div className="sm:mt-2 mr-1 ml-1 mb-1 mt-0 overflow-auto text-wrap">
                                  ...
                                  delectus nisi eveniet accusantium impedit
                                  rerum temporibus, autem veniam illum
                                  recusandae hic?
                                </div>
                              </div>
                            </Popover>
